    ...
    void step()
    {
        for (int i = 0; i < 48; i++)
        {
            ...
            for (int j = 0; j < 48; j += 4)
            {
                ...
                // __m128 d2 = _mm_add_ps(_mm_add_ps(_mm_mul_ps(dx, dx), _mm_mul_ps(dy, dy)), 
                //                        _mm_add_ps(_mm_mul_ps(dz, dz), eps_squares));

                __m128 d2 = _mm_add_ps(_mm_add_ps(_mm_add_ps(_mm_mul_ps(dx, dx), _mm_mul_ps(dy, dy)), _mm_mul_ps(dz, dz)), eps_squares);

                __m128 d3 = _mm_mul_ps(d2, _mm_sqrt_ps(d2));
                __m128 d3_inv = _mm_div_ps(G_times_dt, d3); // G * dt / d3  

                vx_update = _mm_add_ps(vx_update, _mm_mul_ps(_mm_mul_ps(dx, other.mass), d3_inv));
                vy_update = _mm_add_ps(vy_update, _mm_mul_ps(_mm_mul_ps(dy, other.mass), d3_inv));
                vz_update = _mm_add_ps(vz_update, _mm_mul_ps(_mm_mul_ps(dz, other.mass), d3_inv));
            }
